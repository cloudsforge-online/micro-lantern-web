 ...
 *
 * 2. THE ACCENT IS EMBER, BY INDIRECTION. `ui/packages/ui/src/tokens.css:594-596` says Lantern's
 *    own UI forces ember "because amber is also its WARN severity and a surface must not wear the
 *    colour of one of the states it reports". Lantern's registry accent (`surfaces.ts:381`) and
 *    `--cf-warn` (`tokens.css:111`) are the same six digits. The override must therefore exist,
...
describe('the stylesheet', () => {
  ...
  it('cites the instruction it is following, so the next reader can check it', () => {
    assert.match(CSS, /tokens\.css:594-596/)
  })
})

describe('the page names a declared product block', () => {
  it('sets data-cf-product to the real registry key', () => {
    // tokens.css:389-396 says every key an app may set is declared, precisely so a surface cannot
    // fall through to the company ember in silence — which is what `admin` did. `lantern` has a
    // block at tokens.css:596-602, so naming it is both honest and safe.
